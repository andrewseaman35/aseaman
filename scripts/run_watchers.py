...
class Watcher(object):
    ...
    def watch(self):
        ...
        while True:
            ...
            if (
                not self.last_modification_time
                or modification_time > self.last_modification_time
            ):
...
class RunWatchers(BaseScript):
    ...
    def _run(self):
        self.jinja_thread = self._new_watcher_thread(
            "jinja", self.jinja_dir, self.trigger_jinja_build
        )
        self.scss_thread = self._new_watcher_thread(
            "scss", self.scss_dir, self.trigger_scss_build
        )
        self.js_thread = self._new_watcher_thread(
            "js", self.js_dir, self.trigger_js_build
        )
