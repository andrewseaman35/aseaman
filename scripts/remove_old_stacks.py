...
class RemoveOldStacks(BaseScript):
    ...
    def _setup_parser(self):
        ...
        self.parser.add_argument(
            "--stack-name", dest="stack_name", help="cloudformation stack name"
        )
        self.parser.add_argument(
            "--deploy-env", dest="deploy_env", help="test, stage, live"
        )
    ...
    def _run(self):
        ...
        stack_summaries = self.client.list_stacks(StackStatusFilter=STATES_TO_DELETE)[
            "StackSummaries"
        ]
        ...
        print(
            "Deleted {} stack{}".format(
                len(stacks_to_delete), "" if len(stacks_to_delete) == 1 else "s"
            )
        )
