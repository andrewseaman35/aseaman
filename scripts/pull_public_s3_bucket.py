...
class PullPublicS3Bucket(BaseScript):
    ...
    def _setup_parser(self):
        ...
        self.parser.add_argument(
            "--download", help="actually download", action="store_true"
        )
    ...
    def _validate_args(self):
        ...
        if self.args.download:
            confirm = input(
                "this will overwrite any matched files in the destination, you sure? "
            )
        ...
        for result in paginator.paginate(Bucket=bucket, Delimiter="/", Prefix=prefix):
            if result.get("CommonPrefixes") is not None:
                for subdir in result.get("CommonPrefixes"):
                    if not any(
                        subdir.get("Prefix", "").startswith(pref)
                        for pref in OMIT_PREFIXES
                    ):
                        self._download_dir(
                            subdir.get("Prefix"), local, bucket, download=download
                        )
                    ...
                    if self.args.download:
                        try:
                            self.s3_resource.meta.client.download_file(
                                bucket, file.get("Key"), local + file.get("Key")
                            )
