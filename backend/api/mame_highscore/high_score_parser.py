...
def validate_parser_config(config):
    ...
    for game_id, game in games.values():
        ...
        assert game['parser_id'] in PARSERS_BY_ID, f"parser_id {game['parser_id']} not defined"
        if game['post_processor_id']:
            assert game['post_processor_id'] in POST_PROCESSORS_BY_ID, f"post_processor_id {game['post_processor_id']} not defined"
...
class HighScoreParser(object):
    ...
    def parse(cls, game_id, data):
        ...
        sorted_parsed_scores = sorted(parsed_scores, key=lambda score: score['score'], reverse=True)
        final_scores = post_processor(sorted_parsed_scores) if post_processor else sorted_parsed_scores
