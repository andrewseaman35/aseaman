...
resource "aws_lambda_permission" "api_invoke_function" {
  ...
  source_arn = "arn:aws:execute-api:${local.aws_region}:${local.aws_account_id}:${var.rest_api_id}/*/${aws_api_gateway_method.post.http_method}${aws_api_gateway_resource.api_respource.path}"
}
